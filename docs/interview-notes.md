...
## Unit 7: Document Extraction Tool Concepts & Interview Preparation

### Question 39: How does file upload work in FastAPI with `UploadFile` and `multipart/form-data`, and why parse in memory via `io.BytesIO`?
**Answer:**
- Standard JSON endpoints expect a `Content-Type: application/json` body. However, binary files (such as PDFs or DOCX documents) cannot be efficiently sent as raw JSON without bulky Base64 encoding.
- `multipart/form-data` encodes binary files in MIME-delimited parts. FastAPI exposes this via `file: UploadFile = File(...)` (backed by `python-multipart`).
- `UploadFile` exposes an asynchronous file-like interface (`await file.read()`).
- **In-Memory Streaming (`io.BytesIO`):** Rather than writing the uploaded bytes to a temporary directory on the server's hard drive (which risks disk filling, directory traversal attacks, or lingering unencrypted clinical documents), we wrap the raw byte stream in an in-memory buffer (`io.BytesIO(file_bytes)`). Pure-Python parsers read directly from RAM, ensuring maximum speed, clean memory cleanup, and enhanced data privacy.

---

### Question 40: Why choose deterministic Python parsers (`pypdf`, `python-docx`, `email.parser`) over OCR (Tesseract) for this MVP?
**Answer:**
- **Simplicity & Portability:** Optical Character Recognition (OCR) engines like Tesseract require heavy external C/C++ binaries, language trained data packs, and significant CPU/memory resources during image preprocessing.
- **Real-World Digital Documents:** In modern corporate and clinical settings, the vast majority of shared complaint notices (PDF reports, Word memos, emails, text logs) are digitally generated documents containing digital text streams, not hand-scanned photos.
- **Deterministic Transparency:** Using standard libraries provides instant, predictable, zero-latency extraction with no OCR hallucination or character substitution errors.
- If a user uploads a flattened or image-only PDF with zero extractable text streams, the system cleanly identifies the missing text and returns an actionable HTTP 422 alert explaining that the document appears scanned, rather than failing silently.

---

### Question 41: Why must deterministic file validation execute before checking Groq AI credentials?
**Answer:**
- If a client uploads an invalid file—such as an unsupported `.exe` or `.jpg`, an oversized 50 MB document, or an empty 0-byte file—that request is invalid by definition regardless of whether the AI backend is active.
- Validating the file extension, file size ceiling (10 MB), and non-empty content *first* ensures immediate, deterministic rejection with accurate client error codes (HTTP 400, 413, or 422).
- It also conserves server resources, prevents unnecessary AI network calls, and allows the file parsing engine to be tested and verified independently of third-party API availability.

---

### Question 42: What is the architectural difference between EML transmission metadata (`Date` header) and pharmaceutical `complaint_date`?
**Answer:**
- In RFC 822 email messages (.eml), the `Date` header records when the email message was generated by the sending mail client.
- In pharmaceutical quality assurance, `complaint_date` specifically denotes when the customer or patient observed the defect or formally reported it to the manufacturer.
- For example, an email sent on Monday morning might say: *"Last Thursday, our surgical team noticed discoloration in Batch X."*
- If the system automatically mapped the email header date to `complaint_date`, it would record Monday instead of Thursday, misrepresenting the defect timeline and potentially compromising regulatory adverse event reporting deadlines.
- By treating the header date strictly as transmission metadata, `complaint_date` is populated only when the document text explicitly confirms the observation or received date. Otherwise, it correctly remains `null`.

---

### Question 43: Why avoid an arbitrary character count cutoff (such as "<5 chars") when detecting unreadable or scanned PDFs?
**Answer:**
- An arbitrary character threshold risks false rejections. A legitimate, ultra-concise complaint document (e.g. an urgent recall notice containing *"Batch B1 contaminated"*) could be rejected if an arbitrary threshold was set too high.
- Instead of guessing a character length cutoff, a PDF is properly treated as unreadable only when the text extracted across all pages is completely empty or consists entirely of whitespace.
- This accurately differentiates digital text PDFs from image-only or scanned PDFs while ensuring that legitimate concise complaints pass through unimpeded.

---

### Question 44: How does non-destructive form merging protect user data when applying extracted document proposals?
**Answer:**
- When an uploaded document is parsed, incomplete documents will naturally produce `null` for missing fields (e.g., missing batch numbers or unknown manufacturing dates).
- If the QA specialist had already manually filled in the batch number or manufacturing date on the form, a naive merge that blindly overwrites all fields with the AI extraction result would wipe out the user's manual input with `null`.
- Non-destructive form merging enforces that **only non-null, non-empty extracted values** update the Redux form. Any field that the document did not contain leaves the existing form value completely intact.

---

### Question 45: Why is dynamic form submission switching (POST vs. PATCH) essential for AI-assisted workflows?
**Answer:**
- In an AI-assisted complaint lifecycle, intake is not a single one-and-done event. A user may extract facts from text or documents, save the complaint to PostgreSQL to obtain a registered record UUID, and then subsequently issue conversational corrections via AI Edit (or manual form edits).
- If the form only supports `POST /api/complaints`, saving the correction would create an entirely separate row with a new UUID, corrupting batch tracking and duplicate metrics.
- By tracking `savedComplaintId` in client state, the form dynamically switches to **Update Mode** (`PATCH /api/complaints/{id}`). This ensures updates are applied to the existing record in place, preserving audit history and maintaining relational integrity.

---

### Question 46: What is the regulatory significance of maintaining zero automated database writes across all AI operations?
**Answer:**
- Under FDA 21 CFR Part 11 and EU GMP Annex 11, electronic records must have accountable human authorship.
- Large language models are non-deterministic and can produce hallucinations, incorrect field assignments, or miss critical clinical context.
- If an AI model were permitted to autonomously execute `INSERT` or `UPDATE` queries upon receiving an email or text message, invalid or unverified complaints could trigger automated product quarantines or regulatory notifications without human review.
- Keeping AI workflows strictly read-only and in-memory ensures that every database row in PostgreSQL has passed through a human QA verification boundary before being persisted.

---

### Question 47: Why is it critical to preserve existing development test records during product readiness audits?
**Answer:**
- In real-world software engineering and production auditing, testing must prove that new features do not degrade, overwrite, or corrupt preexisting records.
- Preserving historical records validates schema backwards-compatibility and verifies that queries (such as listing or updating records) work correctly against both legacy data and newly created records.
- Deleting test records mid-audit can mask race conditions, foreign key constraints, or timestamp synchronization bugs that only emerge in non-empty databases.

---

### Question 48: Why did RFC 9110 replace `422 Unprocessable Entity` with `422 Unprocessable Content`?
**Answer:**
- RFC 4918 (WebDAV) originally introduced status code 422 with the term "Unprocessable Entity", referring to XML entities.
- In HTTP Semantics specification RFC 9110 (published in 2022), the term "entity" was deprecated throughout the HTTP standard in favor of "content" (representing the payload body).
- Consequently, modern frameworks like Starlette and FastAPI recommend `HTTP_422_UNPROCESSABLE_CONTENT` to adhere to modern HTTP standards while preserving the exact numeric status code 422.

---

## Unit 9 & Submission UI Polish Concepts & Interview Preparation

### Question 49: Why is visual alignment with industry-standard QMS layouts critical in enterprise pharmaceutical software?
**Answer:**
- In regulated life sciences environments, user interface layout is an essential safety and ergonomics consideration (human factors engineering).
- Quality assurance operators process hundreds of deviations and complaints under strict regulatory timelines. Cluttered, non-standard, or highly decorative interfaces increase cognitive fatigue and operator error.
- Adhering to structured, numbered uppercase sections (e.g. `1. ORIGIN & CUSTOMER DETAILS`, `2. PRODUCT & BATCH IDENTIFICATION`) ensures consistent eye tracking, rapid field scanning, and clear orientation between origin facts, batch identification, defect narratives, and risk assessment.
- Clear status badges (such as "Pending Triage" transitioning to "Registered") provide immediate situational awareness of record lifecycle state.

---

### Question 50: How do unified dropzone + text entry intake zones improve operator efficiency without compromising workflow flexibility?
**Answer:**
- In standard complaint workflows, incoming reports arrive in varied formats: some as customer emails, some as phone call notes, and others as vendor PDF or Word attachments.
- By providing a prominent top drag-and-drop dropzone, an intuitive "OR" separator, and a direct text entry area on the primary intake tab, operators can ingest complaints in a single step regardless of source medium.
- At the same time, maintaining specialized dedicated tabs (`Document Upload` and `Edit / Correct`) preserves deep workflow support for multi-document comparisons and conversational prompt-based corrections, balancing immediate accessibility with advanced capabilities.
